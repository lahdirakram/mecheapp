...
// No real filesystem on web (used for the in-browser dev loop): the hooks stay disabled there and
// callers fall back to signed URLs. Constructing Directory at module load crashes on web.
const isWeb = Platform.OS === 'web';
let cacheDirInstance: Directory | null = null;
function cacheDir(): Directory {
  if (!cacheDirInstance) cacheDirInstance = new Directory(Paths.document, 'imgcache');
...
export function useLocalImages(bucket: string, paths: (string | null | undefined)[]) {
  ...
  const list = isWeb ? [] : [...new Set(paths.filter((p): p is string => !!p && !/^https?:\/\//.test(p)))].sort();
...
export function useLocalImage(bucket: string, path: string | null | undefined) {
  const sb = useSupabase();
  const usable = !isWeb && !!path && !/^https?:\/\//.test(path);
  return useQuery({
    queryKey: ['localimg', bucket, path ?? ''],
    enabled: usable,
    staleTime: Infinity,
