// mobile-screens.jsx — 10 B2C screens for Reflet
  ...
  return (
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:MPAL.bg,color:MPAL.ink,position:'relative'}}>
      ...
      <div style={{padding:'18px 26px 32px',display:'flex',flexDirection:'column',gap:14}}>
        <div className="mono" style={{fontSize:10,letterSpacing:'.18em',color:accent,fontWeight:600}}>
          REFLET · BÊTA
        </div>
        <div className="serif" style={{fontSize:44,lineHeight:1.02,letterSpacing:'-0.025em'}}>
          {mt(lang,'tagline').split(',')[0]},<br/>
          <span style={{fontStyle:'italic',color:accent}}>différemment.</span>
        </div>
        <div style={{fontSize:15,color:MPAL.mute,lineHeight:1.45,marginTop:2,maxWidth:300}}>
          {mt(lang,'sub1')}
        </div>
...
// 2. SELFIE ──────────────────────────────────────────────────────────────────
function MScreenSelfie({ lang, accent }) {
  ...
  return (
    <div style={{height:'100%',background:'#0a0908',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{position:'absolute',top:130,left:0,right:0,textAlign:'center',color:'#fff',zIndex:5}}>
        ...
        <div className="mono" style={{fontSize:10,letterSpacing:'.12em',opacity:0.75,marginTop:6}}>
          ÉTAPE 1 / 3
        </div>
      </div>
...
function MScreenFeed({ lang, accent }) {
  const card = MFEED[0];
  return (
    <div style={{height:'100%',position:'relative',background:'#000',overflow:'hidden'}}>
      ...
      {/* bottom gradient */}
      <div style={{position:'absolute',inset:0,
        background:'linear-gradient(to bottom, transparent 30%, rgba(0,0,0,0.7) 100%)',pointerEvents:'none'}}/>
      {/* top: search + filter */}
      <div style={{position:'absolute',top:60,left:0,right:0,padding:'0 16px',
        display:'flex',gap:8,zIndex:5}}>
        {[
          {l:lang==='fr'?'Pour toi':'For you', on:true},
          {l:lang==='fr'?'Suivis':'Following'},
          {l:lang==='fr'?'Tendances':'Trending'},
        ].map((c,i)=>(
          <button key={i} style={{
            padding:'8px 14px',borderRadius:999,fontSize:13,fontWeight:600,cursor:'pointer',
            background: c.on ? '#fff' : 'rgba(0,0,0,0.4)',
            color: c.on ? '#1a1612' : '#fff',
            border: c.on ? 'none' : '1px solid rgba(255,255,255,0.18)',
            backdropFilter: c.on ? 'none' : 'blur(20px)',
          }}>{c.l}</button>
        ))}
      </div>
      {/* right side actions */}
      <div style={{position:'absolute',right:14,bottom:200,display:'flex',flexDirection:'column',gap:18,zIndex:5}}>
        {[
          {ic:'heart',label:card.loves,fill:false},
          {ic:'bookmark',label:lang==='fr'?'1.2k':'1.2k',fill:false},
          ...
          <button key={i} style={{background:'transparent',border:'none',cursor:'pointer',
            ...
            <div style={{width:48,height:48,borderRadius:24,background:'rgba(255,255,255,0.12)',
              backdropFilter:'blur(20px)',display:'flex',alignItems:'center',justifyContent:'center',
              border:'1px solid rgba(255,255,255,0.18)'}}>
              <MIcon name={a.ic} size={22} color="#fff" fill={a.fill?'#fff':'none'}/>
            </div>
      ...
      {/* bottom card info */}
      <div style={{position:'absolute',left:0,right:0,bottom:120,padding:'0 18px',color:'#fff',zIndex:5}}>
        <div className="mono" style={{fontSize:10,letterSpacing:'.14em',
          padding:'4px 10px',borderRadius:999,background:accent,display:'inline-block',marginBottom:10}}>
          {card.tag[lang]}
        </div>
        <div className="serif" style={{fontSize:32,letterSpacing:'-0.02em',lineHeight:1.05}}>
          {card.name[lang]}
        </div>
        <div style={{fontSize:13,opacity:0.85,marginTop:4}}>{card.desc[lang]}</div>
        <div style={{marginTop:14,display:'flex',gap:10}}>
          <button style={{
            flex:1,padding:'13px 18px',borderRadius:999,background:'#fff',color:'#1a1612',
            border:'none',cursor:'pointer',fontSize:15,fontWeight:600,
            display:'flex',alignItems:'center',justifyContent:'center',gap:8,
          }}>
            <MIcon name="sparkle" size={16}/> {mt(lang,'try_look')}
          </button>
          <button style={{
          ...
          }}><MIcon name="bookmark" size={20} color="#fff"/></button>
        </div>
      </div>
      {/* swipe hint */}
      <div style={{position:'absolute',right:14,top:140,color:'#fff',
        display:'flex',flexDirection:'column',alignItems:'center',gap:4,opacity:0.6,zIndex:5}}>
    ...
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:MPAL.bg,color:MPAL.ink,overflow:'hidden'}}>
      ...
      <div style={{padding:'14px 18px 32px',background:`linear-gradient(to top, ${MPAL.bg} 70%, ${MPAL.bg}00)`}}>
        ...
        <div style={{textAlign:'center',marginTop:8,fontSize:11,color:MPAL.mute}}>
          {lang==='fr'?'Utilise 1 crédit · 2 essais restants ce mois':'Uses 1 credit · 2 tries left this month'}
        </div>
      </div>
    </div>
  );
...
// 5. GENERATING ─────────────────────────────────────────────────────────────
function MScreenGenerating({ lang, accent }) {
        ...
        <div style={{marginTop:18,padding:'18px',borderRadius:18,background:MPAL.paper,
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:MPAL.mute,marginTop:2}}>
              {lang==='fr'?'Reflet choisit pour toi':'Reflet picks for you'}
            </div>
          </div>
    ...
    <div style={{height:'100%',background:MPAL.bg,color:MPAL.ink,position:'relative',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{margin:'4px 16px 14px',height:160,borderRadius:18,overflow:'hidden',position:'relative',
          ...
          {[{x:90,y:70,m:96},{x:200,y:95,m:92},{x:280,y:60,m:88},{x:140,y:130,m:81}].map((p,i)=>(
            <g key={i}>
              ...
              <path d={`M${p.x} ${p.y-16} A8 8 0 1 1 ${p.x+0.01} ${p.y-16} L${p.x} ${p.y} Z`}
                fill={i===0?accent:'#1a1612'}/>
              <circle cx={p.x} cy={p.y-14} r="3.5" fill="#fff"/>
            </g>
          ))}
          {/* user pos */}
          <circle cx="180" cy="80" r="6" fill="#4A88E0" stroke="#fff" strokeWidth="2"/>
          <circle cx="180" cy="80" r="18" fill="#4A88E0" opacity="0.18"/>
        </svg>
        ...
        <div style={{position:'absolute',bottom:10,right:10,padding:'5px 10px',borderRadius:999,
          background:'rgba(255,255,255,0.92)',fontSize:11,fontWeight:600,color:'#1a1612'}}>
          {mt(lang,'near_me')}
        </div>
      </div>
    ...
    <div style={{height:'100%',background:MPAL.bg,color:MPAL.ink,position:'relative',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',padding:'0 24px'}}>
        <div style={{
          ...
          background:'#1a1612',position:'relative',
          ...
          <div style={{position:'absolute',inset:0,display:'flex'}}>
            <div style={{flex:1,position:'relative'}}>
              ...
              <div style={{position:'absolute',top:10,left:10,padding:'3px 8px',
                ...
                fontSize:8,fontWeight:700,letterSpacing:'.12em',color:'#1a1612'}}>AVANT</div>
            </div>
            <div style={{flex:1,position:'relative'}}>
              <MPortrait hair="bob" mood="warm" tint={accent}/>
          ...
          <div style={{position:'absolute',left:0,right:0,bottom:0,
            ...
            <div className="serif" style={{fontSize:18,letterSpacing:'-0.01em',fontStyle:'italic'}}>
              Reflet
            </div>
            <div style={{fontSize:9,opacity:0.75,marginTop:2,letterSpacing:'.04em'}}>
      ...
      <div style={{padding:'14px 18px 0'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
          {[
            ...
            {ic:'tiktok',label:'TikTok',bg:'#1a1612',c:'#fff'},
            {ic:'snap',label:'Snap',bg:'#FFFC00',c:'#1a1612'},
          ].map((o,i)=>(
            <button key={i} style={{
  ...
  return (
    <div style={{height:'100%',background:MPAL.bg,color:MPAL.ink,position:'relative',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'4px 18px 110px'}}>
        ...
        {/* credits card */}
        <div style={{
          padding:18,borderRadius:18,background:MPAL.paper,border:`1px solid ${MPAL.border}`,marginBottom:14,
        }}>
          <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',gap:10}}>
            <div className="mono" style={{fontSize:10,letterSpacing:'.14em',color:MPAL.mute,fontWeight:600,whiteSpace:'nowrap'}}>
              {lang==='fr'?'CE MOIS':'THIS MONTH'}
            </div>
            <div className="mono" style={{fontSize:10,letterSpacing:'.08em',color:MPAL.mute,whiteSpace:'nowrap'}}>
              {lang==='fr'?'RENOUV. 1 JUIN':'RENEWS JUN 1'}
            </div>
          </div>
          <div style={{display:'flex',alignItems:'baseline',gap:6,marginTop:6}}>
            <div className="serif" style={{fontSize:42,letterSpacing:'-0.02em',lineHeight:1}}>{MUSER.credits}</div>
            <div style={{fontSize:14,color:MPAL.mute}}>/ {MUSER.free_cap} {mt(lang,'free_trials')}</div>
          </div>
          <div style={{display:'flex',gap:4,marginTop:12}}>
            {Array.from({length:MUSER.free_cap}).map((_,i)=>(
              <div key={i} style={{flex:1,height:6,borderRadius:6,
                background: i<MUSER.credits ? accent : MPAL.border}}/>
            ))}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{position:'absolute',top:-30,right:-30,width:160,height:160,borderRadius:80,
            background:`radial-gradient(circle, ${accent}66, transparent 70%)`}}/>
          <div style={{position:'relative'}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
              <MIcon name="crown" size={18} color={accent} fill={accent} stroke={0}/>
              <div className="mono" style={{fontSize:10,letterSpacing:'.18em',color:accent,fontWeight:600}}>
                REFLET PREMIUM
              </div>
            </div>
            <div className="serif" style={{fontSize:28,letterSpacing:'-0.02em',lineHeight:1.1}}>
              Essais illimités<br/>+ studio signature.
            </div>
            <div style={{fontSize:12,opacity:0.7,marginTop:10,lineHeight:1.5}}>
              {lang==='fr'
                ?'Aperçus illimités, presets exclusifs des grandes maisons, suppression du watermark.'
                :'Unlimited previews, exclusive presets from top houses, watermark-free.'}
            </div>
            <div style={{display:'flex',alignItems:'baseline',gap:6,marginTop:16,flexWrap:'wrap'}}>
              <div className="serif" style={{fontSize:32,letterSpacing:'-0.02em',whiteSpace:'nowrap'}}>€7,99</div>
              <div style={{fontSize:13,opacity:0.6,whiteSpace:'nowrap'}}>/{lang==='fr'?'mois':'mo'}</div>
              <div style={{flex:1,minWidth:8}}/>
              <button style={{
                padding:'10px 16px',borderRadius:999,background:accent,color:'#fff',
                border:'none',cursor:'pointer',fontSize:13,fontWeight:600,
                display:'flex',alignItems:'center',gap:6,whiteSpace:'nowrap',flexShrink:0,
              }}>{mt(lang,'upgrade')} <MIcon name="arrowRight" size={13} color="#fff"/></button>
            </div>
            <div style={{fontSize:10,opacity:0.5,marginTop:10}}>
              {lang==='fr'?'7 jours gratuits puis 7,99€/mois. Annulable à tout moment.':'7 days free, then €7.99/mo. Cancel anytime.'}
            </div>
          </div>
        </div>
...
Object.assign(window, {
  MScreenWelcome, MScreenSelfie, MScreenFeed, MScreenCustomize,
  MScreenGenerating, MScreenResult, MScreenWardrobe, MScreenSalons,
  MScreenShare, MScreenProfile,
});
